#include "tensorflow/compiler/jit/kernels/xla_batch_matcher.h"
#include "tensorflow/core/platform/logging.h"

namespace tensorflow {

XlaBatchMatcher::XlaBatchMatcher() {
    env_str_ = std::getenv(kEnvName);
    parse_env_config();
}
...
void XlaBatchMatcher::parse_env_config() {
  ...
  if (!env_str_) {
    LOG(INFO) << "[XLA_BATCH_WARN] Env var " << kEnvName <<
  ...
  while (std::getline(ss, item, ',')) {
    ...
    try {
      ...
      std::cerr << "[XLA_BATCH_WARN] Failed to parse config item, skipping: " <<
        trimmed_item << " (" << e.what() << ")" << std::endl;
    }
  }
...
int64_t XlaBatchMatcher::find_min_larger_batch(int64_t real_batch) {
  if (real_batch <= 0 || real_batch >= kMaxBatch) {
    std::cerr << "[XLA_BATCH_WARN] Out of valid range: " << real_batch << std::endl;
    return real_batch;
  }
